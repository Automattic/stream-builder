...
namespace TumblrApp\Sniffs\Arrays;
...
/**
 * TumblrApp\Sniffs\Arrays\ArrayDeclarationSniff.
 * Copy of Squiz\Sniffs\Arrays\ArrayDeclarationSniff.
 *
 * A test to ensure that arrays conform to the array coding standard.
 *
 * This is pretty much a carbon copy of the Squiz sniff, with the removal of SingleLineNotAllowed as it caused false positives
 * Also, this now correctly adds a comma after last item of multiline array
 */
class ArrayDeclarationSniff implements Sniff
{
    ...
    public function processSingleLineArray(File $phpcsFile, $stackPtr, $arrayStart, $arrayEnd)
    {
        ...
        if ($valueCount > 0) {
            ...
            foreach ($commas as $comma) {
                ...
                // if we're dropping elements, don't allow spaces between consecutive commas
    ...
    public function processMultiLineArray(File $phpcsFile, $stackPtr, $arrayStart, $arrayEnd)
    {
        ...
        if ($tokens[$lastContent]['line'] === $tokens[$arrayEnd]['line']) {
            ...
            // closing brace should align with start of line
            if ($prev = $phpcsFile->findFirstOnLine(T_WHITESPACE, $stackPtr, true)) {

                $prev_node = $tokens[$prev];

                $column = $prev_node['column'] - 1; // Columns start at 1
                $expected = (int) $column;

                $found = (int) ($tokens[$arrayEnd]['column'] - 1);
        ...
        if ($keyUsed === false && empty($indices) === false) {
            ...
            foreach ($indices as $value) {
                ...
                if ($lastValueLine !== false && $tokens[$value['value']]['line'] === $lastValueLine) {
                    ...
                    $prev = $phpcsFile->findFirstOnLine(array(), $stackPtr-1, true);

                    $prev_node = $tokens[$prev];
                    $column = ($prev_node['column'] + ($prev_node['code'] === T_WHITESPACE ? strlen($prev_node['content']) : 0)) - 1;
                    $expected = $column + 4;

                    $first = $phpcsFile->findFirstOnLine(T_WHITESPACE, $value['value'], true);
                    $found = (int) ($tokens[$first]['column'] - 1);
                    if ($found !== $expected) {
        ...
        */
        ...
        $indicesStart = $keywordStart;

        // Find first token on the line, if whitespace, offset by whitespace
        $prev = $phpcsFile->findFirstOnLine(array(), $stackPtr-1, true);
        if ($prev !== false){
            $prev_node = $tokens[$prev];
            $indicesStart = $prev_node['column'] + ($prev_node['code'] === T_WHITESPACE ? strlen($prev_node['content']) : 0);
        }
        ...
        foreach ($indices as $index) {
            ...
            if ($nextComma === false || ($tokens[$nextComma]['line'] !== $valueLine)) {
                $error = 'Each line in an array declaration should end in a comma';
